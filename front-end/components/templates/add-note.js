...
export async function getStaticProps(context) {
  const res = await fetch(`https://localhost:8080/language-note`)
  ...
  return (
    <div>
      <Form onSubmit={onSubmit} title={"Nova Anotação"}>
        <Input label={"Titulo"} name={"title"} rule={{ required: true }} />
        <Input label={"SubTitulo"} name={"excerpt"} rule={{ required: true }} />
        <Input
      ...
      <ReactMarkdown style={{ width: "40%", float: "right" }}>
        {content}
      </ReactMarkdown>
    </div>
  )
